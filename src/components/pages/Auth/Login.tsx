        ...
        const draw = () => {
            ...
            for (let i = 0; i < N; i++) {
                ...
                for (let j = i + 1; j < N; j++) {
                    ...
                    if (d < 130) { gl.strokeStyle = `rgba(59,130,246,${(1 - d / 130) * .09})`; gl.lineWidth = .35; gl.beginPath(); gl.moveTo(a.x, a.y); gl.lineTo(b.x, b.y); gl.stroke(); }
                }
                gl.beginPath(); gl.arc(a.x, a.y, a.r, 0, 6.283);
                gl.fillStyle = `rgba(59,130,246,${.2 + Math.sin(t + a.p) * .3})`; gl.fill();
            }
...
.ns-inp:focus { border-color: rgba(59,130,246,.45) !important; box-shadow: 0 0 0 3px rgba(59,130,246,.06), 0 0 25px rgba(59,130,246,.04); background: rgba(31,40,51,.55) !important; }
...
export const Login = () => {
    ...
    const fs = (f: string, v: string) => !touched[f] || !v ? 'border-white/[.07]' : validate(f, v) ? 'border-[#3B82F6]/40' : 'border-red-500/40';
    ...
    const VIcon = ({ f, v }: { f: string; v: string }) => { if (!touched[f] || !v) return null; return validate(f, v) ? <Check className="w-3.5 h-3.5 text-[#3B82F6]/70" /> : <X className="w-3.5 h-3.5 text-red-400/70" />; };
    ...
    const toggle = useCallback(() => {
        ...
        // 1. pulse card border
        tl.to(cardRef.current, { boxShadow: '0 0 60px rgba(59,130,246,.12), 0 0 0 1px rgba(59,130,246,.25), 0 40px 100px rgba(0,0,0,.5)', duration: .2 })
            ...
            // 6. settle card glow
            .to(cardRef.current, { boxShadow: '0 0 40px rgba(59,130,246,.05), 0 0 0 1px rgba(59,130,246,.12), 0 40px 100px rgba(0,0,0,.45)', duration: .3 }, '-=.15');
    ...
    const btnStyle = { background: 'linear-gradient(135deg, #2563EB 0%, #3B82F6 50%, #2563EB 100%)', backgroundSize: '200% 200%', boxShadow: '0 4px 25px rgba(59,130,246,.12), 0 0 0 1px rgba(59,130,246,.1), inset 0 1px 0 rgba(255,255,255,.15)', color: '#0B0C10' };
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-[#0B0C10] flex items-center justify-center relative overflow-hidden p-4" style={{ fontFamily: "'Inter', sans-serif" }}>
                <Stars />
                <div className="fixed inset-0 z-[1] pointer-events-none opacity-[.012]" style={{ backgroundImage: 'linear-gradient(rgba(59,130,246,.4) 1px,transparent 1px),linear-gradient(90deg,rgba(59,130,246,.4) 1px,transparent 1px)', backgroundSize: '80px 80px' }} />
                <div className="fixed z-[2] pointer-events-none rounded-full blur-[120px] opacity-15" style={{ width: 600, height: 400, background: 'radial-gradient(ellipse, rgba(59,130,246,.18) 0%, transparent 70%)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} />

                <button onClick={() => nav('/')} className="fixed top-5 left-5 z-50 flex items-center gap-1.5 px-3 py-1.5 text-white/30 hover:text-[#3B82F6] rounded-lg border border-white/[.06] hover:border-[#3B82F6]/20 hover:bg-white/[.02] transition-all text-[10px] font-mono tracking-[.2em] uppercase group backdrop-blur-sm">
                    <ArrowLeft className="w-3 h-3 group-hover:-translate-x-0.5 transition-transform" /> back
                </button>

                {/* ═══ CARD ═══ */}
                <div ref={cardRef} className="relative z-10 w-full max-w-[960px] rounded-3xl overflow-hidden" style={{ opacity: 0, minHeight: 560, backdropFilter: 'blur(20px)', background: 'linear-gradient(135deg, rgba(15,17,24,.9) 0%, rgba(11,12,16,.94) 100%)', boxShadow: '0 0 40px rgba(59,130,246,.05), 0 0 0 1px rgba(59,130,246,.12), 0 40px 100px rgba(0,0,0,.45)' }}>
                    ...
                    <form ref={signUpRef} onSubmit={handleSignUp} className="absolute right-0 top-0 bottom-0 w-full md:w-[55%] flex items-center justify-center px-6 md:px-12 z-[5]">
                        <div className="w-full max-w-[360px]">
                            <div className="ns-si mb-6">
                                <div className="flex items-center gap-2.5 mb-3"><div className="w-9 h-9 rounded-xl bg-[#3B82F6]/[.08] border border-[#3B82F6]/15 flex items-center justify-center"><Sparkles className="w-[17px] h-[17px] text-[#3B82F6]" /></div><span className="text-[9px] font-semibold text-[#3B82F6]/40 tracking-[.25em] uppercase">NovaSketch</span></div>
                            ...
                            <div className="ns-si mt-5 pt-3 border-t border-white/[.05] flex items-center justify-between"><div className="flex items-center gap-1.5 text-[8px] text-white/20 tracking-[.12em] font-mono"><Shield className="w-3 h-3" /> 256-BIT ENCRYPTED</div><div className="flex items-center gap-1.5 text-[8px] text-white/20 tracking-[.12em] font-mono"><div className="w-1 h-1 rounded-full bg-[#3B82F6]/50 animate-pulse" /> LIVE</div></div>
                            <button type="button" onClick={toggle} disabled={busy} className="md:hidden mt-4 w-full text-center text-xs text-white/40 hover:text-[#3B82F6]">Already have an account? <span className="text-[#3B82F6]/80 font-medium">Sign In →</span></button>
                        </div>
                    </form>
                    ...
                    <form ref={signInRef} onSubmit={handleSignIn} className="absolute left-0 top-0 bottom-0 w-full md:w-[55%] flex items-center justify-center px-6 md:px-12 z-[5]" style={{ visibility: 'hidden', opacity: 0 }}>
                        <div className="w-full max-w-[360px]">
                            <div className="mb-6">
                                <div className="flex items-center gap-2.5 mb-3"><div className="w-9 h-9 rounded-xl bg-[#3B82F6]/[.08] border border-[#3B82F6]/15 flex items-center justify-center"><Fingerprint className="w-[17px] h-[17px] text-[#3B82F6]" /></div><span className="text-[9px] font-semibold text-[#3B82F6]/40 tracking-[.25em] uppercase">NovaSketch</span></div>
                            ...
                            <div className="mt-5 pt-3 border-t border-white/[.05] flex items-center justify-between"><div className="flex items-center gap-1.5 text-[8px] text-white/20 tracking-[.12em] font-mono"><Shield className="w-3 h-3" /> SECURE SESSION</div><div className="flex items-center gap-1.5 text-[8px] text-white/20 tracking-[.12em] font-mono"><div className="w-1 h-1 rounded-full bg-[#3B82F6]/50 animate-pulse" /> ONLINE</div></div>
                            <button type="button" onClick={toggle} disabled={busy} className="md:hidden mt-4 w-full text-center text-xs text-white/40 hover:text-[#3B82F6]">Don't have an account? <span className="text-[#3B82F6]/80 font-medium">Sign Up →</span></button>
                        </div>
                    </form>

                    {/* ═══ ACCENT PANEL — slides left↔right ═══ */}
                    <div ref={accentRef} className="hidden md:flex absolute top-0 bottom-0 w-[45%] z-[15] items-center justify-center overflow-hidden" style={{ left: '0%', background: 'linear-gradient(155deg, #0a192f 0%, #0f2f44 30%, #184858 55%, #1d6b6b 80%, #2563EB 100%)' }}>
                        {/* mesh blobs */}
                        <div className="absolute inset-0 overflow-hidden pointer-events-none">
                            <div className="absolute w-[260px] h-[260px] rounded-full opacity-25 blur-[80px]" style={{ background: 'radial-gradient(circle, #3B82F6 0%, transparent 70%)', top: '8%', left: '15%', animation: 'nsMesh1 12s ease-in-out infinite' }} />
                            <div className="absolute w-[200px] h-[200px] rounded-full opacity-20 blur-[70px]" style={{ background: 'radial-gradient(circle, #2563EB 0%, transparent 70%)', bottom: '12%', right: '8%', animation: 'nsMesh2 10s ease-in-out infinite 2s' }} />
                            <div className="absolute w-[140px] h-[140px] rounded-full opacity-15 blur-[60px]" style={{ background: 'radial-gradient(circle, #3B82F6 0%, transparent 70%)', top: '50%', left: '-5%', animation: 'nsMesh1 14s ease-in-out infinite 4s' }} />
                        </div>
                        {/* orbital rings */}
                        <div className="absolute w-[280px] h-[280px] rounded-full border border-white/[.04] pointer-events-none" style={{ animation: 'nsOrbit 28s linear infinite', top: '50%', left: '50%', marginTop: -140, marginLeft: -140 }}><div className="absolute -top-1 left-1/2 w-1.5 h-1.5 rounded-full bg-[#3B82F6]/30 -ml-[3px]" /></div>
                        <div className="absolute w-[180px] h-[180px] rounded-full border border-white/[.03] border-dashed pointer-events-none" style={{ animation: 'nsOrbit 20s linear infinite reverse', top: '50%', left: '50%', marginTop: -90, marginLeft: -90 }}><div className="absolute -bottom-1 left-1/2 w-1 h-1 rounded-full bg-[#3B82F6]/20 -ml-[2px]" /></div>
                        {/* floating shapes */}
                        ...
                        <div className="absolute bottom-4 left-5 text-[7px] font-mono text-white/15 z-20"><div>$ session.init()</div><div className="text-[#3B82F6]/30">→ ready</div></div>
